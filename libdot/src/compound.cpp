...
extern int arrowTailClip (edge_t*, point*, int, int, bezier*, int);
extern int arrowHeadClip (edge_t*, point*, int, int, bezier*, int);
...
boxIntersect (point pp, point cp, box* bp)
{
  ...
  /* failure */
  {
    ...
    fprintf (stderr, "Error: segment [%s,%s] does not intersect box ll=%s,ur=%s\n", 
...
getCluster (graph_t* g, char* cluster_name)
{
  ...
  if (sg == NULL)
    fprintf (stderr, "Warning: cluster named %s not found\n", cluster_name);
  return sg;
}
...
countVertCross (pointf* pts, int xcoord)
{
  ...
  sign = old_sign = SGN(pts[0].x, xcoord);
  for (i = 1; i <= 3; i++) {
    sign = SGN(pts[i].x, xcoord);
    if (sign != old_sign) num_crossings++;
    old_sign = sign;
  }
...
countHorzCross (pointf* pts, int ycoord)
{
  ...
  sign = old_sign = SGN(pts[0].y, ycoord);
  for (i = 1; i <= 3; i++) {
    sign = SGN(pts[i].y, ycoord);
    if (sign != old_sign) num_crossings++;
    old_sign = sign;
  }
 ...
 * Invariant: spline is composed of only one part, 
...
makeCompoundEdge (graph_t* g, edge_t*  e)
{
  ...
  int      starti, endi;     /* index of first and last control point */
  box*     bb;
  int      i, j;
  int      size;
  point    pts[4];
  ...
  if (e->u.spl->size > 1) {
    fprintf (stderr, "Warning: %s -> %s: spline size > 1 not supported\n",
  ...
  size = bez->size;
    ...
    /* allocate new Bezier */
  nbez = (bezier*)malloc(sizeof(bezier));
  ...
  if (lh) {
    bb = &(lh->u.bb);
    if (!inBox (e->head->u.coord, bb)) {
      fprintf (stderr, "Warning: %s -> %s: head not inside head cluster %s\n",
    ...
    if (inBox (bez->list[0], bb)) {
      if (inBox (e->tail->u.coord, bb)) {
        fprintf (stderr, "Warning: %s -> %s: tail is inside head cluster %s\n",
          e->tail->name, e->head->name, agget (e, "lhead"));
        return;
      }
      ...
      if (bez->eflag)
        endi = arrowHeadClip (e, bez->list, starti, 0, nbez, bez->eflag);
      endi += 3;
    }
    else {
      ...
      if (endi == size - 1) {  /* no intersection */
        ...
        if (bez->eflag)
          endi = arrowHeadClip (e, bez->list, starti, endi, nbez, bez->eflag);
        endi += 3;
      }
    }
  }
  ...
  if (lt) {
    bb = &(lt->u.bb);
    if (!inBox (e->tail->u.coord, bb)) {
      fprintf (stderr, "Warning: %s -> %s: tail not inside tail cluster %s\n",
    ...
    if (inBox (bez->list[endi], bb)) {
      if (inBox (e->head->u.coord, bb)) {
        fprintf (stderr, "Warning: %s -> %s: head is inside tail cluster %s\n",
          e->tail->name, e->head->name, agget (e, "ltail"));
        return;
      }
      ...
      if (bez->sflag)
        starti = arrowTailClip (e, bez->list, starti, endi-3, nbez, bez->sflag);
    }
    else {
  ...
  nbez->list = (point*)malloc(nbez->size * sizeof(point));
