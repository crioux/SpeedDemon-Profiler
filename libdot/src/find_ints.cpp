...
int gt(const struct vertex **i,const struct vertex **j);
...
case -1:  /* forward edge, test and insert	*/	
	...
	_new = (struct active_edge *) malloc(sizeof(struct active_edge));
	if (all.number == 0) { all.first = _new; _new->last = 0; } /* insert */
		else { all.final->next = _new; _new->last = all.final; }

	_new->name = templ;	 _new->next = 0;
	templ->active = _new; all.final = _new;	 all.number++;
