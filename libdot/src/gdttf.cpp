...
fontFetch ( char **error, void *key )
{
	...
	font_t			*a;
	fontkey_t		*b=(fontkey_t *)key;
	int			i, n, map_found, font_found=0;
	unsigned short		platform, encoding;
	char                    *fontsearchpath, *fontpath, *fontname;
	char			*fullname = NULL;
        char                    *name, *path, *dir;
	char                    *strtok_ptr;

	a = (font_t *)gdMalloc(sizeof(font_t));
	a->fontname = strdup(b->fontname);
	a->ptsize = b->ptsize;
	a->angle = b->angle;
	a->sin_a = sin(a->angle);
	a->cos_a = cos(a->angle);
	a->engine = b->engine;
        ...
        fontname = strdup(a->fontname);

        /*
         * Must use gd_strtok_r else pointer corrupted by strtok in nested loop.
         */
        for (name = gd_strtok_r(fontname,LISTSEPARATOR,&strtok_ptr); name;
	...
	if ((err = TT_Open_Face(*b->engine, fullname, &a->face))) {
	...
	if (TT_Set_Instance_Resolutions(a->instance, GD_RESOLUTION, GD_RESOLUTION)) {
...
glyphFetch ( char **error, void *key )
{
	...
	a = (glyph_t *)gdMalloc(sizeof(glyph_t));
...
bitmapFetch ( char **error, void *key )
{
	...
	a = (bitmap_t *)gdMalloc(sizeof(bitmap_t));
	a->xoffset = b->xoffset;
	a->yoffset = b->yoffset;

	b->glyph->Bit.bitmap = a->bitmap = (char *)gdMalloc(b->glyph->Bit.size);
		...
		double ptsize, double angle, int x, int y, char *string)
{
	...
	if (font->have_char_map_Sjis) {
#endif
		if (tmpstr = (unsigned char *)gdMalloc(BUFSIZ)) {
