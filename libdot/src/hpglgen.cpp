...
static void hpgl_begin_page(point page, double scale, int rot, point offset)
{
    ...
    if (rot == 90) {
      ...
      Origin.x = (int)(PB.LL.y + scale*offset.y - HP_OY);
      Origin.y = (int)(PageWidth - PB.LL.x - scale*offset.x - HP_OX);
    } 
    else {
      ...
      Origin.x = (int)(PB.LL.x + scale*offset.x - HP_OX);
      Origin.y = (int)(PB.LL.y + scale*offset.y - HP_OY);
    }
        /* Set clipping window */
