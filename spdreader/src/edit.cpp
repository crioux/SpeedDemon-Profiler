...
BEGIN_EVENT_TABLE (Edit, wxScintilla)
    ...
    EVT_MENU (ID_FIND_TEXT,           Edit::OnFindText)
	...
	// ui
	EVT_UPDATE_UI(wxID_COPY, Edit::OnEditCopyUpdateUI)
	EVT_UPDATE_UI(wxID_CUT, Edit::OnEditCutUpdateUI)
	EVT_UPDATE_UI(wxID_PASTE, Edit::OnEditPasteUpdateUI)
	EVT_UPDATE_UI(ID_VIEW_FIND, Edit::OnFindUpdateUI)
	EVT_UPDATE_UI(ID_VIEW_FIND_NEXT, Edit::OnFindNextUpdateUI)
	EVT_UPDATE_UI(ID_FIND_TEXT, Edit::OnFindTextUpdateUI)
...
void Edit::OnFindText(wxCommandEvent &event)
{
	wxComboBox *pft=(wxComboBox *)g_theApp->GetMainFrame()->FindWindow(ID_FIND_TEXT);
	if(pft==NULL)
	{
...
void Edit::OnFindTextUpdateUI(wxUpdateUIEvent &event)
{
	event.Enable(true);
}


void Edit::DoFind(void)
{
	int cnt=GetLength();

	int pos=FindText(GetCurrentPos()+1,cnt,m_strFindText,0);
	if(pos<0)
	{
		pos=FindText(0,GetCurrentPos(),m_strFindText,0);
	}
	if(pos>=0)
	{
		m_nFindPosition=pos;
		SetCurrentPos(pos);
		SetSelection(pos,pos+((int)m_strFindText.Len()));
		return;
	}
...
void Edit::OnEditCutUpdateUI(wxUpdateUIEvent &evt)
{
	evt.Enable(!GetReadOnly() && GetSelectionStart()<GetSelectionEnd());
}

void Edit::OnEditCopyUpdateUI(wxUpdateUIEvent &evt)
{
	evt.Enable(GetSelectionStart()<GetSelectionEnd());
}

void Edit::OnEditPasteUpdateUI(wxUpdateUIEvent &evt)
{
	evt.Enable(CanPaste());
}

void Edit::OnFindUpdateUI(wxUpdateUIEvent &evt)
{
	evt.Enable(true);
}

void Edit::OnFindNextUpdateUI(wxUpdateUIEvent &evt)
{
