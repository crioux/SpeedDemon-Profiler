...
void wxCompositeShape::Copy(wxShape& copy)
{
  ...
  oglObjectCopyMapping[this]=&compositeCopy;
  ...
  while (node)
  {
    ...
    oglObjectCopyMapping[object]=newObject;
  ...
  while (node)
  {
    ...
    wxShape *newConstraining = oglObjectCopyMapping.find(constraint->m_constrainingObject)->second;
    ...
    while (node2)
    {
      ...
      wxShape *newConstrained = oglObjectCopyMapping.find(constrainedObject)->second;
  ...
  {
    wxDivisionShape *division = (wxDivisionShape *)(node->GetData());
	stdext::hash_map<void *, wxShape *>::iterator node1 = oglObjectCopyMapping.find(division);
    stdext::hash_map<void *, wxShape *>::iterator leftNode;
    stdext::hash_map<void *, wxShape *>::iterator topNode;
    stdext::hash_map<void *, wxShape *>::iterator rightNode;
    stdext::hash_map<void *, wxShape *>::iterator bottomNode;
    if (division->GetLeftSide())
      leftNode = oglObjectCopyMapping.find(division->GetLeftSide());
    if (division->GetTopSide())
      topNode = oglObjectCopyMapping.find(division->GetTopSide());
    if (division->GetRightSide())
      rightNode = oglObjectCopyMapping.find(division->GetRightSide());
    if (division->GetBottomSide())
      bottomNode = oglObjectCopyMapping.find(division->GetBottomSide());
    if (node1!=oglObjectCopyMapping.end())
    {
      wxDivisionShape *newDivision = (wxDivisionShape *)(node1->second);
      if (leftNode!=oglObjectCopyMapping.end())
        newDivision->SetLeftSide((wxDivisionShape *)(leftNode->second));
      if (topNode!=oglObjectCopyMapping.end())
        newDivision->SetTopSide((wxDivisionShape *)(topNode->second));
      if (rightNode!=oglObjectCopyMapping.end())
        newDivision->SetRightSide((wxDivisionShape *)(rightNode->second));
      if (bottomNode!=oglObjectCopyMapping.end())
        newDivision->SetBottomSide((wxDivisionShape *)(bottomNode->second));
    }
    node = node->GetNext();
  }
}
