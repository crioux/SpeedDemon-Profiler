...
void wxShape::Recentre(wxDC& dc)
{
  ...
  size_t noRegions = m_regions.GetCount();
  for (size_t i = 0; i < noRegions; i++)
  {
    wxNode *node = m_regions.Item(i);
    if (node)
    {
...
{
  size_t n = GetNumberOfTextRegions();
  wxString buff;
  for (size_t i = 0; i < n; i++)
  {
    if (parentName.Length() > 0)
      buff << parentName << wxT(".") << (unsigned int)i;
    else
      buff << (unsigned int)i;
    SetRegionName(buff, (int)i);
  }
  wxNode *node = m_children.GetFirst();
  int j = 0;
  while (node)
...
void wxShape::FindRegionNames(wxStringList& list)
{
  ...
  for (size_t i = 0; i < n; i++)
  {
    wxString name(GetRegionName((int)i));
    list.Add(name);
  }
