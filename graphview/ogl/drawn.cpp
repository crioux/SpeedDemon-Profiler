...
wxOpSetGDI::wxOpSetGDI(int theOp, wxPseudoMetaFile *theImage, size_t theGdiIndex, int theMode):
  wxDrawOp(theOp)
{
...
void wxOpSetGDI::Do(wxDC& dc, double WXUNUSED(xoffset), double WXUNUSED(yoffset))
{
  switch (m_op)
  {
    case DRAWOP_SET_PEN:
    {
      ...
      // colour
      if (m_image->m_outlineColours.find(m_gdiIndex)!=m_image->m_outlineColours.end())
      {
...
void wxPseudoMetaFile::DrawPolygon(int n, wxPoint pts[], int flags)
{
    ...
    if (flags & oglMETAFLAGS_OUTLINE)
        m_outlineOp = (int)(m_ops.GetCount() - 1);
}
...
void wxPseudoMetaFile::SetPen(wxPen* pen, bool isOutline)
{
    ...
    int n = (int)m_gdiObjects.GetCount();
