...
void wxPolygonShape::OnDraw(wxDC& dc)
{
    ...
    if (m_shadowMode != SHADOW_NONE)
    {
      ...
      dc.DrawPolygon((int)n, intPoints, WXROUND(m_xpos + m_shadowOffsetX), WXROUND(m_ypos + m_shadowOffsetY));
    }
    ...
    if (m_brush)
      dc.SetBrush(* m_brush);
    dc.DrawPolygon((int)n, intPoints, WXROUND(m_xpos), WXROUND(m_ypos));
...
void wxPolygonShape::OnDrawOutline(wxDC& dc, double x, double y, double w, double h)
{
  ...
  size_t n = m_originalPoints->GetCount();
  wxPoint *intPoints = new wxPoint[n];
  size_t i;
  for (i = 0; i < n; i++)
  {
  ...
  dc.DrawPolygon((int)n, intPoints, WXROUND(x), WXROUND(y));
  delete[] intPoints;
}
...
{
  size_t maxN = (m_points ? (m_points->GetCount() - 1) : 0);
