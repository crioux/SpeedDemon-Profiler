...
void wxLineShape::FormatText(wxDC& dc, const wxString& s, int i)
{
	...
	wxStringListNode *strnode = string_list->GetFirst();
	while (strnode)
	{
		char *s = (char *)strnode->GetData();
    ...
    case 0:
		{
			...
			size_t n = m_lineControlPoints->GetCount();
			size_t half_way = (n/2);
    ...
    case ARROW_METAFILE:
		{
			if (arrow->GetMetaFile())
			{
				...
				*/
				...
				{
					wxFAIL_MSG("Unknown arrowhead rotation case in lines.cc");
				}
...
void wxLineShape::OnTranslate(double xoff, double yoff)
{
	SetX(GetX()+xoff);
	SetY(GetY()+yoff);
...
void wxLineShape::OnDraw(wxDC& dc)
{
	if (m_lineControlPoints)
	{
		...
		if (m_isSpline)
			dc.DrawSpline((int)n, points);
		else
			dc.DrawLines((int)n, points);
...
void wxLineShape::ReadAttributes(wxExpr *clause)
{
	...
	if (line_list)
	{
		...
		wxExpr *node = line_list->value.GetFirst;
		
		while (node)
		{
			wxExpr *xexpr = node->value.GetFirst;
			double x = xexpr->RealValue();
			
			wxExpr *yexpr = xexpr->GetNext;
	...
	if (arrow_list)
	{
		wxExpr *node = arrow_list->value.GetFirst;
