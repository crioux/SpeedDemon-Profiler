...
// Modified by:
// Created:     12/07/98
// RCS-ID:      $Id: basic.cpp,v 1.4 2003/02/06 02:35:32 dildog Exp $
...
void wxShapeEvtHandler::OnTranslate(double xoff, double yoff)
{
	if(m_previousHandler)
		m_previousHandler->OnTranslate(xoff,yoff);
}
...
void wxShape::OnTranslate(double xoff, double yoff)
{
	SetX(GetX()+xoff);
	SetY(GetY()+yoff);
}
...
void wxShape::ReadAttributes(wxExpr *clause)
{
	...
	if (strings && strings->Type() == wxExprList)
	{
		...
		wxExpr *node = strings->value.GetFirst;
		while (node)
		{
			...
			{
				wxExpr *first = string_expr->value.GetFirst;
				wxExpr *second = first ? first->GetNext : (wxExpr*) NULL;
				wxExpr *third = second ? second->GetNext : (wxExpr*) NULL;
...
void wxShape::ReadRegions(wxExpr *clause)
{
	...
	*
		...
		*/
		...
		{
			wxExpr *node = textExpr->value.GetFirst;
			while (node)
			{
				wxExpr *string_expr = node;
				double the_x = 0.0;
